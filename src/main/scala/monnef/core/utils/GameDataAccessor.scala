...
import cpw.mods.fml.common.{Loader, FMLCommonHandler, FMLLog}
...
object GameDataAccessor {
  ...
  def registerBlock(block: Block, blockName: String): Int = {
    val mainGameData = getMain
    val registerBlockMethod = ReflectionHelper.findMethod(classOf[GameData], mainGameData, Array("registerBlock"), classOf[Block], classOf[String])
    registerBlockMethod.invoke(mainGameData, block, blockName).asInstanceOf[Int]
  }
  ...
  def iItemRegistryAdd(idHint: Integer, name: String, item: Item, availabilityMap: java.util.BitSet): Int = {
    ...
    val prefix = Loader.instance().activeModContainer().getModId
    addMethod.invoke(itemReg, idHint, prefix + ":" + name, item, availabilityMap).asInstanceOf[Int]
  }
