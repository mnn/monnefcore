...
public abstract class BlockMachine extends BlockMonnefCore {
    ...
    public TileMachine getTile(IBlockAccess world, int x, int y, int z) {
        return (TileMachine) world.getTileEntity(x, y, z);
    }
    ...
    public void onBlockPlacedBy(World w, int x, int y, int z, EntityLivingBase entity, ItemStack stack) {
        ...
        if (useDefaultDirection) {
            ...
            if (obj != null && obj.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                MonnefCorePlugin.Log.printDebug("No hit, using default side.");
            }
        } else {
    ...
    public int getRenderType() {
        if (customRenderer) {
            return getUseOwnRenderId() ? this.renderID : MonnefCoreNormalMod.renderID;
        } else {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float par7, float par8, float par9) {
        ...
        if (hand != null) {
            ...
            if (WrenchHelper.isPipeWrenchOrCompatible(item)) {
                ...
                TileMachine machineTile = (TileMachine) world.getTileEntity(x, y, z);
