...
object BlockHelper {
  ...
  def rotateBoundingBoxCoordinates(box: BoundingBoxSize, rotation: Int, x: Int, y: Int, z: Int, addCurrentLocation: Boolean): AxisAlignedBB = {
    ...
    rotation match {
      case 0 => AxisAlignedBB.getBoundingBox(bbx, bby, bbz, bbxx, bbyy, bbzz)
      case 3 => AxisAlignedBB.getBoundingBox(bbz, bby, 1 - bbxx, bbzz, bbyy, 1 - bbx)
      case 2 => AxisAlignedBB.getBoundingBox(1 - bbxx, bby, 1 - bbzz, 1 - bbx, bbyy, 1 - bbz)
      case 1 => AxisAlignedBB.getBoundingBox(1 - bbzz, bby, bbx, 1 - bbz, bbyy, bbxx)
      case _ => AxisAlignedBB.getBoundingBox(bbx - 1, bby, bbz - 1, bbxx + 1, bbyy, bbzz + 1)
    }
  }
